...
class MqttClient:
    def __init__(self, config):
        ...
        if self.availability_topic:
            ...
            _LOGGER.debug("Setting STATUS to: %s" % topic)
            self.mqttc.will_set(topic, payload=GATEWAY_OFFLINE, qos=2, retain=True)
    ...
    def publish(self, messages):
        ...
        for m in messages:
            if m.use_global_prefix:
                ...
                topic = m.topic
            self.mqttc.publish(topic, m.payload, qos=1, retain=m.retain)
    ...
    def on_connect(self, client, userdata, flags, rc):
        if self.availability_topic:
            self.publish(
                [
                    MqttMessage(
                        topic=self.availability_topic, payload=GATEWAY_ONLINE, retain=True
                    )
                ]
            )
    ...
    def __del__(self):
        if self.availability_topic:
            self.publish(
                [
                    MqttMessage(
                        topic=self.availability_topic, payload=GATEWAY_OFFLINE, retain=True
                    )
                ]
            )
